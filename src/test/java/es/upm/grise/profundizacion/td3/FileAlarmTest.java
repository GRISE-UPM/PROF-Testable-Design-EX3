...
public class FireAlarmTest {
	...
	public void setUp() throws ConfigurationFileProblemException, DatabaseProblemException, IOException {
		...
		mockResult = mock(JsonNode.class);
		path=path.replace("\\","\\\\");
	...
	public void TempetureHightest() throws Exception,SensorConnectionProblemException, IncorrectDataException, DatabaseProblemException, ConfigurationFileProblemException {
		FireAlarm alarm = spy(FireAlarm.class); // Crear directamente el spy sin usar el constructor real
		doReturn(81).when(alarm).getTemperature(anyString()); // Stubear el método getTemperature
	...
	La creación de la variable private ObjectMapper mapper=new ObjectMapper(); psara poder crear un mock en los tet y así poder probar InvalidJsonTest.
